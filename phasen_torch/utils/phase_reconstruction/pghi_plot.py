...
class Pghi_Plot(object):
    ...
    def __init__(self, show_plots=True, show_frames=5, pre_title='', soundout='./soundout/',
                 plotdir='./pghi_plots/', Fs=44100, verbose=True, logfile='log.txt'):
        '''
        parameters:
            show_plots
...
#             startfreqs = [startpoints[0][1]]
    ...
    def subplot(self, figax, sigs, r, c, p, elev, azim, mask, startpoints, fontsize=None):
        ...
        for i, s in enumerate(sigs):
            ...
            if mask is None:  #plot all values
                ...
                indices = np.where(self.limit(mask[mintime:maxtime, minfreq:maxfreq]) is True)
    ...
    def quiver(self, title, qtuples, mask=None, startpoints=None):
        ...
        ax.quiver(x,y,z,u,v,w,length=.5, arrow_length_ratio=.3, pivot='tail',
                  color=self.colors[1], normalize=True)
        if startpoints is not None:
            for stpt in startpoints:
                n = stpt[0]
                m = stpt[1]
