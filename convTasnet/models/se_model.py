...
from .convTasnetModel import ConvTasNet
...
class Net(nn.Module):
  def __init__(self, mode, device):
    ...
    self.mode = mode
    self.device = device
    self._net_model = ConvTasNet(num_sources=1, enc_kernel_size=PARAM.convTas_enc_kernel_size,
                                 causal=PARAM.convTas_causal)  # [N, 1, L] -> [N, 1, L]
  ...
  def forward(self, mixed_wav_batch):
    ...
    est_wav_batch = self._net_model(mixed_wav_batch.unsqueeze(1)).squeeze(1) # [N, L] -> [N, L]

    est_stft_batch = self._stft_fn(est_wav_batch) # [N, 2, F, T]
  ...
  def get_losses(self, est_wav_features:WavFeatures, clean_wav_batch):
    ...
    if "sisnr2" in all_losses:
      self.sisnr2 = losses.batchMean_sisnrLossV2(est_clean_wav_batch, self.clean_wav_batch)
...
if __name__ == "__main__":
    ...
    device = 'cpu'
    test_model = ConvTasNet(num_sources=1, enc_kernel_size=PARAM.convTas_enc_kernel_size)
    wav_batch = torch.rand(1, PARAM.sampling_rate).to(device)
    # emb_batch = torch.rand(1, PARAM.speaker_emb_size).to(device)
    macs, params = profile(test_model, inputs=(wav_batch.unsqueeze(1),))
    ...
    model = nn.Conv2d(4, 16, (3,3), stride=2, bias=False, groups=1)
    flops, params = get_model_complexity_info(model, (4, 3, 6),
                                              as_strings=False,
