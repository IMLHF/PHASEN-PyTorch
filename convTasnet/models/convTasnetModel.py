...
class DepthConv1d(nn.Module):
    ...
    def __init__(self, input_channel, hidden_channel, kernel, padding, dilation=1, skip=True, last_layer=False, causal=False):
            ...
            for i in range(layer):
                if self.dilated:
                    self.TCN.append(DepthConv1d(BN_dim, hidden_dim, kernel, dilation=2**i, padding=2**i, skip=skip,
                                                last_layer=(s == stack-1 and i == layer-1), causal=causal))
                else:
    ...
    def forward(self, input):
        ...
        if self.skip:
            ...
            for i in range(len(self.TCN)):
                ...
                if i != len(self.TCN)-1: # if last_layer and skip, residual=None
...
# Conv-TasNet
# (self, enc_dim=512, feature_dim=128, sr=16000, win=2, layer=8, stack=3,
...
class ConvTasNet(nn.Module):
    def __init__(self,
        ...
        # hyper parameters
        self.num_spk = num_sources

        self.enc_dim = enc_num_feats
        self.feature_dim = msk_num_feats
        self.hidden_feature_dim = msk_num_hidden_feats

        self.win = enc_kernel_size
        self.stride = self.win // 2

        self.layer = msk_num_layers
        self.stack = msk_num_stacks
        self.kernel = msk_kernel_size
        ...
        # TCN separator
        self.TCN = TCN(self.enc_dim, self.enc_dim*self.num_spk, self.feature_dim, self.hidden_feature_dim,
