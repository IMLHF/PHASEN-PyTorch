...
def main():
  ...
  # print(device)
  se_model_fn = se_model.Net(PARAM.MODEL_TRAIN_KEY, device)

  ckpt_lst = [str(_dir) for _dir in list(ckpt_dir.glob("*.ckpt"))]
  if len(ckpt_lst) > 0:
    ckpt_lst.sort()
    se_model_fn.load(ckpt_lst[-1])
    misc_utils.print_log("load ckpt %s\n" % ckpt_lst[-1])
  else:
    for name, param in se_model_fn.named_parameters():
    ...
    evalOutputs_prev = eval_one_epoch(se_model_fn, val_batch_iter)
    misc_utils.print_log("                                            "
                         "                                            "
                         "                                         \n",
  ...
  s_epoch = se_model_fn.start_epoch
  ...
  for epoch in range(s_epoch, max_epoch+1):
    ...
    # train
    trainOutputs = train_one_epoch(se_model_fn, train_batch_iter, train_log_file)
    ...
    # validation
    evalOutputs = eval_one_epoch(se_model_fn, val_batch_iter)
    ...
    # test
    testOutputs = test_one_epoch(se_model_fn)
    ...
    se_model_fn.save_every_epoch(str(ckpt_dir.joinpath(ckpt_name)))
    evalOutputs_prev = evalOutputs
    msg = "  ckpt(%s) saved.\n" % ckpt_name
    misc_utils.print_log(msg, train_log_file)
    msg = "  Nan grad batch %d\n" % se_model_fn._nan_grads_batch
    misc_utils.print_log(msg, train_log_file)
