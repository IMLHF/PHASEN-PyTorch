...
class Dual_Transformer_v2(nn.Module):
    """
    Deep duaL-path RNN.
    args:
    ...
    def __init__(self, input_size, output_size, dropout=0, num_layers=1):
        super(Dual_Transformer_v2, self).__init__()
    ...
    def forward(self, input):
        ...
        if input.requires_grad:
          for i in range(len(self.row_trans)):
            output = checkpoint(self.onelayer, torch.tensor(i), output)
        else:
          for i in range(len(self.row_trans)):
            output = self.onelayer(i, output)
